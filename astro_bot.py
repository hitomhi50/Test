...
def get_gemini_content(sign, date_str):
    ...
    # 使用 v1beta 版本以支援 responseMimeType 功能
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
    
    prompt = (
        f"你是專業占星師。請為今日（{date_str}）的{sign}撰寫一份運勢。\n"
        f"要求：\n"
        f"1. 描述 (description)：精煉 2-3 句。\n"
    ...
    if "error" in res_data:
        # 如果 v1beta 失敗，嘗試回退到不帶 schema 的基本請求
        print("v1beta 請求失敗，嘗試備用方案...")
        url_alt = f"https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
        payload_alt = {"contents": [{"parts": [{"text": prompt + " (請直接輸出 JSON，不要包含 markdown 標籤)"}]}]}
        response = requests.post(url_alt, json=payload_alt)
        res_data = response.json()
...
def post_to_threads(content):
    ...
    print(f"📡 正在嘗試發布貼文至 Threads (User ID: {THREADS_USER_ID})")
